...
class FileController extends Controller
{
    ...
    {
        $this->route = "dashboard.files.";
        $this->view = "dashboard.files.";
        $this->fileService = new FileService();
    }

    public function index(Request $request)
    {
        $response = $this->fileService->index($request);
    ...
    {
        $result = $this->fileService->show($id);
        if (!$result->success) {
    ...
    {
        try {
            $response = $this->fileService->store($request);
            if (!$response->success) {
    ...
    {
        try {
            $response = $this->fileService->update($request, $id);
            if (!$response->success) {
                return ResponseHelper::apiResponse(false, $response->message , null, null, $response->code);
            }
